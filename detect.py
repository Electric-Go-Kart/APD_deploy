...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if pt:
        ...
        if pb:  # https://www.tensorflow.org/guide/migrate#a_graphpb_or_graphpbtxt
            ...
            import tflite_runtime.interpreter as tflite
            interpreter = tflite.Interpreter(model_path=w, experimental_delegates=[tflite.load_delegate('libedgetpu.so.1', options={"device": ":0"})])
    ...
    frame_shape_shm = SharedMemory(name="frame_shape0", create=True, size=frame_ndim*4) #4 bytes per dim as long as int32 is big enough
    ...
    frame_buffer_shm = SharedMemory(name="frame_buffer0", create=True, size=frame_nbytes)
